...
from avb.vae.train import train
from avb.vae.test import test
from avb.decoders import get_decoder
from avb.vae.models import get_encoder
...
def main():
    ...
    if args.is_train:
        ...
        train(encoder, decoder, x_train, x_val, config)
    else:
        x_test = inputs.get_inputs('test', config)
        test(encoder, decoder, x_test, config)
