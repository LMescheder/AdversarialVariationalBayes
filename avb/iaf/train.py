...
from avb.iaf import IAFVAE
from tqdm import tqdm

def train(encoder, decoder, iaf_layers, x_train, x_val, config):
    ...
    if is_anneal:
        ...
        beta = 1
    iaf_train = IAFVAE(encoder, decoder, iaf_layers, x_train, z_sampled, config, beta=beta)
    iaf_val = IAFVAE(encoder, decoder, iaf_layers, x_val, z_sampled, config, is_training=False)
    ...
    train_op = get_train_op(
        iaf_train.loss, encoder_vars + decoder_vars, config
    )


    # Summaries
    summary_op = tf.summary.merge([
        tf.summary.scalar('train/loss', iaf_train.loss),
        tf.summary.scalar('train/ELBO', iaf_train.ELBO_mean),
        tf.summary.scalar('train/KL', iaf_train.KL_mean),
        tf.summary.scalar('train/reconstr_err', iaf_train.reconst_err_mean),
        tf.summary.scalar('val/ELBO', iaf_val.ELBO_mean),
        tf.summary.scalar('val/KL', iaf_val.KL_mean),
        tf.summary.scalar('val/reconstr_err', iaf_val.reconst_err_mean),
    ])
    ...
    with sv.managed_session() as sess:
        ...
        # For interpolations
        z_out = sess.run(iaf_val.z_real, feed_dict={x_val: samples})
        ...
        for batch_idx in progress:
            ...
            ELBO_out, ELBO_val_out = sess.run([iaf_train.ELBO_mean, iaf_val.ELBO_mean])
